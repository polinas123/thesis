...
IRF <- lapply(seq_along(TOPICS), function(i){
        x = cumsum(c(1, ARMAtoMA(ar = S_ARIMA_BASIC[[i]][[j]]$model$phi, ma = S_ARIMA_BASIC[[i]][[j]]$model$theta, lag.max = h)))
})

IRF <- setNames(IRF, TOPICS)
```
...
interesting = c(8,12,18,25,31,37,27,28,29,30)

pltly <- plotly::plot_ly(type = "scatter", mode = "lines") 
for (i in 1: length(TOPICS[interesting])){
        pltly <- plotly::add_trace(p = pltly, y = byTopic[[interesting[i]]][[6]], x = byTopic[[interesting[i]]][[1]], name = TOPICS[interesting][i])
}
...
texts = lapply(seq_along(TOPICS), FUN = function(i){
        ...
        #                 DT[.(CATEGORIES[i]), .(title, sent_text, category_proba), by = article_id], 
        #                 options = list(autoWidth = T))))
        datatable(DT[!is.na(TOPICS[i]), .(publish_date, site, title),], 
                  options = list(autoWidth = T))
})
```
