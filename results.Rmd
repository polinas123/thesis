...
plotIRF <- function(topicID, comparisons = "", h = 6){
        lapply(topicID, function(i) {
                if (comparisons == ""){
                        ir <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[1]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[1]]$model$theta, lag.max = h)))
                        plot(y = ir, x = c(0:h), type = "l", lwd = 2, main = TOPICS[[i]], ylab = "Shock", xlab = "Days", xlim = range(0,h), ylim = range(0,1), sub = skewness(ir)+kurtosis(ir))
                } else if(comparisons == "states"){ 
                        black <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[1]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[1]]$model$theta, lag.max = h)))
                        blue <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[5]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[5]]$model$theta, lag.max = h)))
                        red <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[6]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[6]]$model$theta, lag.max = h)))
                        purple <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[7]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[7]]$model$theta, lag.max = h)))
                        

                        plot( y = black,x = c(0:h), type = "l", lwd = 2,  ylab = "Shock", xlab = "Days", xlim = range(0,h), ylim = range(0,1), main = TOPICS[[i]]) 
                        plot( y = blue,x = c(0:h), type = "l", lwd = 2, ylab = "Shock", xlab = "Days", xlim = range(0,h), ylim = range(0,1), col = "blue") 
                        plot( y = red, x = c(0:h), type = "l", lwd = 2, ylab = "Shock", xlab = "Days", xlim = range(0,h), ylim = range(0,1), col = "red")
                        plot( y = purple, x = c(0:h), type = "l", lwd = 2, ylab = "Shock", xlab = "Days", xlim = range(0,h), ylim = range(0,1), col = "purple")
                                                                                     
                } else if (comparisons == "sites"){
                        black <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[1]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[1]]$model$theta, lag.max = h)))
                        orange <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[3]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[3]]$model$theta, lag.max = h)))
                        green <- cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[4]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[4]]$model$theta, lag.max = h)))
                        ...
                        qplot(data = sites, y = Shock, x = Days, group = Color, colour = Color, main = TOPICS[[i]]) + geom_line() + scale_color_manual(values = levels(sites$Color)) + theme(legend.position='none') + scale_x_discrete(breaks = c(1:h), limits = as.character(c(1:h)), labels = as.character(c(1:h)))       
                }                
        })
}
...
a <- plotIRF(13, comparisons = "sites")
b <- plotIRF(14, comparisons = "sites")

plot_grid(a[[1]],b[[1]])

a <- plotIRF(32, comparisons = "sites")
b <- plotIRF(8, comparisons = "sites")

plot_grid(a[[1]],b[[1]])

a <- plotIRF(7, comparisons = "sites")
b <- plotIRF(2, comparisons = "sites")

plot_grid(a[[1]],b[[1]])

plotIRF(36, comparisons = "sites")
...
a <- plotIRF(37, comparisons = "sites")
...
plotIRF(2, comparisons = "states")
plotIRF(24, comparisons = "states")
...
SKEWENESS <- lapply(seq_along(TOPICS), function(i){
        x = cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[3]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[3]]$model$theta, lag.max = h)))
        return(skewness(x))
})
```
...
PEAKS <- lapply(seq_along(TOPICS), function(i){
        x = cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[j]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[j]]$model$theta, lag.max = h)))
...
DAYS <- lapply(seq_along(TOPICS), function(i){
        x = cumsum(c(1, ARMAtoMA(ar = SADJ_ARIMA_BASIC[[i]][[j]]$model$phi, ma = SADJ_ARIMA_BASIC[[i]][[j]]$model$theta, lag.max = h)))
        return(which(abs(diff(x)) < 0.005)[1])
})
```
