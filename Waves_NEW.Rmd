...
byTopic <- lapply(seq_along(TOPICS), function(i){
        setDates(byTopic[[i]], DATES, cols = colnames(byTopic[[i]])[10:14])
})
```
...
byTopics[[1]] <- melt(data = byTopics[[1]], measure.vars = colnames(byTopics[[i]][10:14]))
# byTopics[[1]][, value := na.fill(value,0)]
```
```{r descriptive}
DT[, sapply(.SD, summary), .SDcols = ]
```


```{r plot}

# shiny descriptive plots
