...
```{r fit}
ACTUALvsFITTED <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                plot(S_ARIMA_BASIC[[i]][[j]]$x, type = "l")
                lines(S_ARIMA_BASIC[[i]][[j]]$fitted, col = "red")
        })
        return(r)
})
```
...
SERIALCORR <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                bgtest(S_ARIMA_BASIC[[i]][[j]]$residuals ~ 1, order = 7)        
        })
        names(r) <- scores
        return(r)
})
...
ARCH <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                ArchTest(S_ARIMA_BASIC[[i]][[j]]$residuals, lags = 7)        
        })
        names(r) <- scores
        return(r)
})
...
NORM <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                JarqueBeraTest(S_ARIMA_BASIC[[i]][[j]]$residuals, robust = T)        
        })
        names(r) <- scores
        return(r)
})
