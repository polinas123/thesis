...
```{r arima_basic}
ARp2_1_0 <- lapply(seq_along(TOPICS), function(i){
        byScore <- lapply(scores, function(j){
                t <- ts(byTopic_m[[i]][variable == j, value])
                model <- auto.arima(y = t, d = 1, max.q = 0, max.p = 2)
        })
})
```
...
SERIALCORR <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                bgtest(ARp2_1_0[[i]][[j]]$residuals ~ 1, order = 7)        
        })
        names(r) <- scores
        return(r)
})
...
ARCH <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                ArchTest(ARp2_1_0[[i]][[j]]$residuals, lags = 7)        
        })
        names(r) <- scores
        return(r)
})
...
NORM <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                JarqueBeraTest(ARp2_1_0[[i]][[j]]$residuals, robust = T)        
        })
        names(r) <- scores
        return(r)
})
...
RESID_ACF <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                Acf(ARp2_1_0[[i]][[j]]$residuals, plot = F)        
        })
        return(r)
})
```
...
RESID_PACF <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                Acf(ARp2_1_0[[i]][[j]]$residuals, plot = F, type = "partial")        
        })
        return(r)
})
```
...
IRF_ARp2_1_0 <- lapply(seq_along(TOPICS), function(i){
        
        byScore <- lapply(seq_along(scores), function(j){
                res <- tryCatch({
                        irf(ARp2_1_0[[i]][[j]]$coef, h = 7)},
                        error=function(e) e)
                p <- if (is.numeric(res)){
                        plot(res, type = "b", ylim = range(0:2), ylab = "Response", xlab = "Horizon", main = paste(TOPICS[i], scores[j], sep = " - "))}
                return(p)
        })
})
```