...
SERIALCORR <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                bgtest(ARIMA_BASIC[[i]][[j]]$residuals ~ 1, order = 7)        
        })
        names(r) <- scores
        return(r)
})
...
ARCH <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                ArchTest(ARIMA_BASIC[[i]][[j]]$residuals, lags = 7)        
        })
        names(r) <- scores
        return(r)
})
...
NORM <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                JarqueBeraTest(ARIMA_BASIC[[i]][[j]]$residuals, robust = T)        
        })
        names(r) <- scores
        return(r)
})
...
RESID_ACF <- lapply(seq_along(TOPICS), function(i){
        r <- lapply(1:7, function(j){
                Acf(ARIMA_BASIC[[i]][[j]]$residuals, plot = F)        
        })
        return(r)
})
```
...
irf <- function(phi, theta = 0, h = 20){
        ...
        if(all(abs(phi) < 1)){ # & (all(abs(theta) < 1)))
                print(paste(phi, "not invertible", sep = " "))}
