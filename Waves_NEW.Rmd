...
```{r arima}
ARIMA <- lapply(seq_along(TOPICS[interesting]), function(i){
        lapply(scores, function(j){
                ...
                benchmark <- auto.arima(y = t)
                weekday <- auto.arima(y = t, xreg = dt[, c(weekdays(as.Date("2017-07-16")+1:6))])
                weekend <- auto.arima(y = t, xreg = dt$weekend)
                holiday <- auto.arima(y = t, xreg = dt$holiday)

        return(list(benchmark$aic, weekday$aic, weekend$aic, holiday$aic))
        })
})
```
